...
from common import ResourceSnapshot, list_print
...
def run_simulator(num_processors, num_memory, jobs, scheduler):
    simulator = Simulator(jobs, num_processors, num_memory, scheduler)
    simulator.run()
    print_simulator_stats(simulator)
    return simulator

def print_simulator_stats(simulator):
    simulator.scheduler.resource_snapshot.restore_old_slices()
