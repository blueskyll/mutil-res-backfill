...
class Job(object):
    def __init__(self, id, user_estimated_run_time, actual_run_time, num_required_processors, num_required_memory, \
            submit_time=0, admin_QoS=0, user_QoS=0, user_id=0): # TODO: are these defaults used?

        assert num_required_processors > 0, "job_id=%s"%id
        assert num_required_memory > 0, "job_id=%s"%id
        ...
        self.num_required_memory = num_required_memory
...
class ValidatingMachine(Machine):
    ...
    machine usage
    """
    def __init__(self, num_processors, num_memory, event_queue):
    ...
    def _add_job(self, job, current_timestamp):
        ...
        assert job.num_required_memory <= self.free_memory
        self.jobs.add(job)
    ...
    def free_memory(self):
        return self.num_memory - self.busy_memory
    ...
    def busy_memory(self):
        return sum(job.num_required_memory for job in self.jobs)
...
def _job_input_to_job(job_input, total_num_processors, total_num_memory):
    # if job input seems to be problematic  
    if job_input.run_time <= 0 or job_input.num_allocated_processors <= 0 or job_input.submit_time < 0 or job_input.used_memory <= 0:
        return Job(
            id = job_input.number,
            user_estimated_run_time = 1,
            actual_run_time = 1, 
            num_required_processors = max(1, job_input.num_allocated_processors),
            num_required_memory = max(1, job_input.used_memory),
    ...
    return Job(
        ...
        num_required_memory = max(min(job_input.used_memory, total_num_memory), 1),
        submit_time = job_input.submit_time, 
        user_id = job_input.user_id, 
    )

def _job_inputs_to_jobs(job_inputs, total_num_processors, total_num_memory):
    for job_input in job_inputs:
        yield _job_input_to_job(job_input, total_num_processors, total_num_memory)
